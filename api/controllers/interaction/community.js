...
exports.add = (req, res) => {
  ...
  if (req.session.userId) {
    if (description.length >= 2 && description.length <= 8192) {
      ...
      res.status(400).json({error: "The description length needs to be at least 2 characters and max 8192 characters."});
    }
    } else {
