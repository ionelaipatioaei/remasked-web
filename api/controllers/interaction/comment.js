...
exports.add = (req, res) => {
  ...
  if (req.session.userId) {
    if (content.length >= 2 && content.length <= 8192) {
      ...
      res.status(400).json({error: "The comment length needs to be at least 2 characters and max 8192 characters."});
    }
  } else {
    res.status(401).json({error: "You need to be authenticated in order to add a comment!"});
  }
}
...
exports.edit = (req, res) => {
  ...
  if (req.session.userId) {
    if (editedText.length >= 2 && editedText.length <= 8192) {
      const cacheKey = `post:${refPost}:user:${req.session.userId}`;
