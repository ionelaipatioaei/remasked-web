...
exports.profile = (mode) => {
  return (req, res) => {
    ...
    const getProfileInfo = async (ownProfile, username, type, next) => {
      ...
      await db.query(query, queryParams, (error, result) => {
        if (!error && result.rows.length) {
          ...
          if (mode === "render") {
            next(data.username, type, () => res.render("navigation/profile", {logged: req.session.userId !== undefined, you: true, ...data}));
          } else {
            next(data.username, type, () => res.json(data));
          }
        } else {
          if (mode === "render") {
            res.render("misc/notFound", {logged: req.session.userId !== undefined});
    ...
    const getProfileData = async (username, type, next) => {
      let query = "";
    ...
    // the only case left is (!name)
      ...
      if (mode === "render") {
        return res.render("navigation/profile", {logged: req.session.userId !== undefined, ...data, error: "You can't view posts/comments!"});
      } else {
        return res.json({...data, error: "You can't view posts/comments"});
      }
    }
  }
}